...
import ca.mss.rd.job.JobPoolAbstract;
...
public final class RdChart2dMan implements RdChartManager {
	...
	enum ChartImplementation {		// In DEBUG mode
		// 									2 traces	8 traces	All +15 BlMM indics
		...
		THREAD_PER_CHART(1,1000000),// 		4 sec		4 sec		7sec  - just 2 threads !!!
	...
	private SmartMap<String, JobPoolAbstract<Point>> drawJob = new SmartMap<String, JobPoolAbstract<Point>>(){
		@Override
		public JobPoolAbstract<Point> valueFactory(Object key) {
			JobPoolAbstract<Point> jobPool;
			
			if( CI.threadSize == 1 )
				jobPool = new JobPoolSequential<Point>("ChartSeq", CI.queueSize, CI.threadPriority);
			else
				jobPool = new JobPoolParallel<Point>("ChartPar", CI.queueSize, CI.threadSize, CI.threadPriority);
			
			jobPool.startPool(); // started as created
			
			assert( Logger.CHART.isOn ? Logger.CHART.printf("%d) Create new job pools=%d for %s", poolCounter.incrementAndGet(), CI.threadSize, key): true);
	...
	@SuppressWarnings("serial")
	private final SmartMap<String, Set<ITrace2D>> traceByQuote = new SmartMap<String, Set<ITrace2D>>(){
		@Override
		public Set<ITrace2D> valueFactory(Object key) {
	...
	@SuppressWarnings("serial")
	private final SmartMap<String, Set<String>> traceByChart = new SmartMap<String, Set<String>>(){
		@Override
		public Set<String> valueFactory(Object key) {
	...
	@SuppressWarnings("serial")
	static public SmartMap<String, Integer[]> counter = new SmartMap<String, Integer[]>(){
		@Override
		public Integer[] valueFactory(Object key) {
	...
	public void drawSpecialPoint(DrawPoint dp) {
		drawJob.get(dp.traceName).queueJob(new Point(dp, SPECIAL_POINT)); 
	}
	...
	public void setTimingFinish(){
		isLast = true;
		assert( Logger.CHART.isOn ? Logger.CHART.printf("FINISH DRAW #1: %d sec", (System.currentTimeMillis() - starttime)/1000): true);
	}
	...
	public void setTimingStart(){
		...
		if( CI == ChartImplementation.ParentThread ){
			assert( Logger.CHART.isOn ? Logger.CHART.printf("START DRAW"): true);
		}
	}
	...
	public void drawRegularPoint(DrawPoint dp){
		...
		JobPoolAbstract<Point> pool = null;
		...
		if( pool.isBusy() ){
			...
			pool.queueJob(new Point(dp, REGULAR_POINT)); 
		}
	}
		
	/*
	 * Paint graphs on separate threads
	 */
	...
	private class Point extends AbstractJob {
		...
		public void executeJob() {
			...
			if( Logger.CHART.isOn && jobCounter.decrementAndGet() == 0 && isLast ){
				synchronized( this ){
					if( isLast ){
						isLast = false;
						assert( Logger.CHART.isOn ? Logger.CHART.printf("FINISH DRAW #2: %d sec", (System.currentTimeMillis() - starttime)/1000): true);
					}
				}
			}
		}
