...
/**
 * ContinuationArrayList uses arrays internally to represent the banching
 * structure. Arrays are compacted when null items are created.
 */
public class ContinuationArrayList extends ContinuationListImpl implements ContinuationList {
	...
	public Move setMove(int index, Move element) {
		Move move = branches[index];
	...
	/* set() ************************************************************ */
	/*
	 * //FIXME: not implemented public Move set (int i, Move m) { Move old =
	 * null;
	 * 
	 * if (i == 0 && branches == null) { branches = new Move[1]; branches[0] =
	 * m; } else if (i < size()) { old = branches[i]; branches[i] = m;
	 * compressVariations(); } else if (m != null) add(m, false);
	 * 
	 * return old; }
	 */
	...
	public void add(Move m, boolean isMain) {
		...
		assert (departureMove == null || departureMove.getHistory().getCurrentMove() == departureMove) : "You cannot add a continuation to moves other than the "
	...
	public boolean add(Move m) {
		Move[] tmp = null;
		assert (departureMove.getHistory().getCurrentMove() == departureMove) : "You cannot add a continuation to moves other than the "
				+ "last move played on the board (History.getCurrentMove())";
		if (branches == null) {
			branches = new Move[1];
			branches[0] = m;
	...
	/**
	 * compresses the branches array so there are no nulls in the list except
	 * for possibily the main line (0). If the list is already full the list is
	 * not affected. If the list is empty then no action occurs.
	 * 
	...
	public Move removeMove(Move m) {
		return removeMove(getIndex(m));
	}
	...
	public void removeAll() {
		if (branches != null)
			...
			for (int i = 0; i < branches.length; i++) {
				...
				if (branches[i] == null)
					continue; // mss: Getting screwed vie multiple
								// removeLastMove()
