...
public class ChessBoard implements Board {
   ...
   // mss[2007.11.22]: Very efficient way to iterate all pieces in two shoots
   public Iterator<ChessPiece> getTeamIterator(boolean isBlack){
	   return isBlack? blackTeam.iterator(): whiteTeam.iterator();
   }
   ...
   public ChessBoard (boolean defaultBoard) {
      squares = new Square[MAX_FILE][MAX_RANK];
         ...
         whiteTeam = new ArrayList<ChessPiece>(16);
         blackTeam = new ArrayList<ChessPiece>(16);
...
//FIXME: not removing moves to keep threats available
   ...
   public ChessPiece[] getThreats (Square sq, boolean isBlack) {
	 ...
	 if (staleLegalDests)
	    genLegalDests();
         ...
         attackers = new LinkedList<ChessPiece>();
   ...
   public ChessPiece[] getGuards (ChessPiece piece) {
        ...
        // mss2011: very strange logic 
        //return getThreats(piece.orig, !piece.isBlack);
        return getGuards(piece.orig, piece.isBlack); 
   ...
   public List<ChessMove> getLegalMoves () {
      List<ChessMove> list = new LinkedList<ChessMove>();
      
      // mss2011: Test Checkmate !!!
      if( this.isCheckmate() )
    	  return list;
   ...
   public Square getOrigin (byte piece_index, int file, int rank, 
      ...
      List<ChessPiece> movingTeam = (isBlackMove) ? blackTeam : whiteTeam;
      List<ChessPiece> dupes = new ArrayList<ChessPiece>(1);
      ChessPiece   piece = null;
	 ...
	 if (((piece.getIndex() % ChessPiece.BLACK_OFFSET) == piece_index) 
	    ...
	    if ((orig_f < 1 && orig_r < 1)
	       ...
	       if (++count > 1) {
	          if (dupes == null) {
		     dupes = new ArrayList<ChessPiece>(2);
		     dupes.add(mover);
		  }
	          dupes.add(piece);
	       }
      ...
      if (!found) { // mss2011: extra messaging
    	  String str = "";
         if (Log.debug) {
		    Log.debug(DEBUG, 
		    		str = "Illegal Move " 
