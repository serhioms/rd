...
public class UtilValidate {
    ...
    public static boolean isDateAfterToday(String date) {
        ...
        java.util.Date now = UtilDateTime.now();
        if (passed != null) {
	...
	public static boolean isValidDate(String str, String format) {
		try {
			str = str.trim();
			Date parseDate = UtilDateTime.parse(str, format);
			return UtilDateTime.format(parseDate, format).indexOf(str) != -1;
		}catch(Throwable t ){
			/* It does not matter why but the format is wrong */
			return false;
		}
